...
namespace Zeroit.Framework.Button.Editor
{
    ...
    public class ShapeInputEditor : System.Drawing.Design.UITypeEditor
    {
        /// <summary>
        ...
        public override object EditValue(System.ComponentModel.ITypeDescriptorContext context,
            ...
            object value)
        {
            if (value is ShapeInput)
            {
                ShapeControlDialog dialog = new ShapeControlDialog((ShapeInput)value);
                //dialog.Show();

                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    return dialog.ShapeInput;
                }
            }
            return value;
        }
        ...
        public override void PaintValue(PaintValueEventArgs e)
        {
            ...
            if (e.Value is ShapeInput)
            {
                Shapes shapeType = ((ShapeInput) e.Value).Shape;
                Brush br = ((ShapeInput)e.Value).GetUITypeEditorBrush(e.Bounds);
                Pen pn = ((ShapeInput)e.Value).GetUITypeEditorPen(e.Bounds);
                bool drawBorder = ((ShapeInput) e.Value).DrawBorder;
                bool colored = ((ShapeInput)e.Value).ColorShape;

                float startAngle = ((ShapeInput) e.Value).StartAngle;
                float sweepAngle = ((ShapeInput)e.Value).EndAngle;

                Point center = new Point(e.Bounds.Width/2, e.Bounds.Height/2);

                Point[] Poly1 = GetPolygonPoints
                (
                    ((ShapeInput)e.Value).PolygonSides,
                    8,
                    ((ShapeInput)e.Value).PolygonStartingAngle,
                    center
                );
